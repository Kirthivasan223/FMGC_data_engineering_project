...
base_path = f's3://sports-bar-db-223/full_load/{data_source}/*.csv'
print(base_path)
...
delta_table = DeltaTable.forName(spark, "fmgc_data.1_gold.dim_products")
df_child_products = spark.sql(f"SELECT product_code, division, category, product, variant FROM fmgc_data.1_gold.sb_dim_products;")
